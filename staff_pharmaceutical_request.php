...
<body>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        <div class="metric-card" role="button" data-bs-toggle="modal" data-bs-target="#allRequestsModal">
                            ...
                            <div class="metric-value" id="totalRequestsCount"><?php echo number_format($total_requests); ?></div>
                    ...
                    <div class="col-md-6">
                        <div class="metric-card" role="button" data-bs-toggle="modal" data-bs-target="#totalClientsModal">
                            ...
                            <div class="metric-value" id="totalClientsCount"><?php echo number_format($total_clients); ?></div>
                            <div class="metric-detail">Click to view details</div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.getElementById('filterSpecies').addEventListener('change', function() {
            console.log('Species filter changed to:', this.value);
            applyFilters();
        });
        ...
        function applyFilters() {
            ...
            console.log('Filter results:', { visibleRequests, visibleClients: visibleClients.size });
            updateStatsCards(visibleRequests, visibleClients.size);
        }
        
        // Function to update stats cards based on visible rows
        function updateStatsCards(totalRequests, totalClients) {
            const totalRequestsCard = document.getElementById('totalRequestsCount');
            const totalClientsCard = document.getElementById('totalClientsCount');
            
            console.log('Updating stats cards:', { totalRequests, totalClients });
            console.log('Found cards:', { totalRequestsCard, totalClientsCard });
            
            if (totalRequestsCard) {
                totalRequestsCard.textContent = totalRequests.toLocaleString();
                console.log('Updated total requests card to:', totalRequests);
            } else {
                console.log('Could not find total requests card');
            }
            
            if (totalClientsCard) {
                totalClientsCard.textContent = totalClients.toLocaleString();
                console.log('Updated total clients card to:', totalClients);
            } else {
                console.log('Could not find total clients card');
            }
        }
        ...
        // Update modals based on current filters
        ...
        function updateAllRequestsModal() {
            ...
            tableRows.forEach(row => {
                ...
                if (searchMatch && speciesMatch && monthMatch) {
                    ...
                    // Remove the action column for modal display
                    const actionCell = originalRow.cells[5];
            ...
            if (!hasData) {
                modalBody.innerHTML = '<tr><td colspan="5" class="text-center">No requests found</td></tr>';
            }
        }
        ...
        function updateTotalClientsModal() {
            ...
            tableRows.forEach(row => {
                ...
                if (searchMatch && speciesMatch && monthMatch) {
                    const clientNameText = row.cells[0].textContent.trim();
                    if (!visibleClients.has(clientNameText)) {
                        visibleClients.add(clientNameText);
                        
                        // Create a new row for this client
                        const newRow = document.createElement('tr');
                        newRow.innerHTML = `
                            <td>${clientNameText}</td>
                            <td>N/A</td>
                            <td>N/A</td>
                        `;
