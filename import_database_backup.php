...
try {
    ...
    if (isset($_POST['import'])) {
        ...
        // Step 2: Fix auto-increment for the 3 tables we'll populate
        echo "<div class='step'>";
        echo "<h4>Step 2: Fixing Auto-Increment Values</h4>";
        ...
        $tables_to_fix = [
            'clients' => 'client_id',
            'livestock_poultry' => 'animal_id',
            'pharmaceutical_requests' => 'request_id'
        ];
        
        foreach ($tables_to_fix as $table => $id_column) {
            // Get current max ID
            ...
            if ($result) {
                ...
                if ($max_id === null || $max_id == 0) {
                    // Table is empty or has issues, reset to 1
                    $next_id = 1;
                } else {
                    $next_id = $max_id + 1;
                }
                ...
                $alter_result = $conn->query("ALTER TABLE $table AUTO_INCREMENT = $next_id");
                
                if ($alter_result) {
                    echo "<p class='info'>✓ Fixed $table: max_id=$max_id, next_id=$next_id</p>";
                } else {
                    echo "<p class='error'>✗ Failed to fix $table: " . $conn->error . "</p>";
                }
            }
        }
        ...
        echo "<div class='step'>";
        echo "<h4>Step 3: Removing Transactions</h4>";
        ...
        // Step 4: Add more clients based on existing patterns
        echo "<div class='step'>";
        echo "<h4>Step 4: Adding More Clients</h4>";
        
        // Check current clients table structure
        $result = $conn->query("SHOW CREATE TABLE clients");
        if ($result) {
            $row = $result->fetch_assoc();
            echo "<details><summary>Debug: Clients table structure</summary><pre>" . htmlspecialchars($row['Create Table']) . "</pre></details>";
        }
        
        // Check current max ID before adding
        $result = $conn->query("SELECT MAX(client_id) as max_id, COUNT(*) as count FROM clients");
        if ($result) {
            $row = $result->fetch_assoc();
            echo "<p class='info'>Before adding: clients count={$row['count']}, max_id={$row['max_id']}</p>";
        }
        
        $client_count = addMoreClients($conn);
        echo "<p class='success'>✓ Added $client_count new client records</p>";
        
        echo "</div>";
        
        // Step 5: Add livestock and poultry records
        echo "<div class='step'>";
        echo "<h4>Step 5: Adding Livestock and Poultry</h4>";
        ...
        // Step 6: Add pharmaceutical requests for anomaly detection
        echo "<div class='step'>";
        echo "<h4>Step 6: Adding Pharmaceutical Requests for ML</h4>";
        ...
        echo "<li><strong>Fix auto-increment</strong> for clients, livestock_poultry, pharmaceutical_requests</li>";
        echo "<li><strong>Remove transactions</strong> (NOT populated)</li>";
        echo "<li><strong>Add clients</strong> (50 new records)</li>";
        echo "<li><strong>Add livestock & poultry</strong> (100 new records)</li>";
        echo "<li><strong>Add pharmaceutical requests</strong> (150 new records)</li>";
        echo "</ol>";
    ...
    for ($i = 0; $i < 50; $i++) { // Add 50 more clients
        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                ...
                if ($count == 0) {
                    error_log("Client insert error: " . $stmt->error . " | Data: $full_name, $contact_number");
                }
            }
            $stmt->close();
        }
