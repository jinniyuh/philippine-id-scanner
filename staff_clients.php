...
$pending_clients_result = $conn->query("SELECT COUNT(*) as count FROM clients c 
                                    WHERE c.status != 'Complied' 
                                    AND EXISTS (SELECT 1 FROM livestock_poultry lp WHERE lp.client_id = c.client_id AND lp.source = 'Disseminated')" . $where_barangay_cond);
...
// Get all clients with animal status (main table)
$clients_query = "SELECT c.*, 
    CASE 
        WHEN c.status = 'Complied' THEN 'Complied'
        WHEN EXISTS (SELECT 1 FROM livestock_poultry lp WHERE lp.client_id = c.client_id AND lp.source = 'Disseminated') THEN 'Pending'
        ELSE 'No Animals'
    END as display_status
FROM clients c" . $where_barangay_total . " ORDER BY c.full_name ASC";
...
<body>
                ...
                <div class="row g-3 stats-section">
                    ...
                    <div class="col-md-3">
                        <div class="metric-card" data-bs-toggle="modal" data-bs-target="#compliedClientsModal" title="Click to view complied clients">
                            ...
                            <div class="metric-value" id="compliedClientsCount"><?php echo number_format($complied_clients); ?></div>
                    ...
                    <div class="col-md-3">
                        <div class="metric-card" data-bs-toggle="modal" data-bs-target="#pendingClientsModal" title="Click to view pending clients">
                            ...
                            <div class="metric-value" id="pendingClientsCount"><?php echo number_format($pending_clients); ?></div>
                            <div class="metric-detail">Has disseminated animals</div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="metric-card" data-bs-toggle="modal" data-bs-target="#noAnimalsClientsModal" title="Click to view clients with no disseminated animals">
                            <div class="metric-title">No Animals</div>
                            <div class="metric-value" id="noAnimalsClientsCount"><?php echo number_format($none_clients); ?></div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    $statusClass = '';
                                    switch($status) {
                                        case 'Complied': $statusClass = 'status-complied'; break;
                                        case 'Pending': $statusClass = 'status-pending'; break;
                                        case 'No Animals': $statusClass = 'status-no-animals'; break;
                                        default: $statusClass = 'status-pending';
                                    }
                                    ?>
                                    <span class="stats-sublabel <?= $statusClass; ?>">
    ...
    <div class="modal fade" id="pendingClientsModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Pending Clients with Disseminated Animals (<?php echo number_format($pending_clients); ?>)</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
    ...
    <!-- No Animals Clients Modal -->
    <div class="modal fade" id="noAnimalsClientsModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
                  ...
                  <tbody>
                    <?php while ($client = $none_clients_list->fetch_assoc()): ?>
                      <tr>
                        ...
                        <td><?php echo htmlspecialchars($client['barangay']); ?></td>
                        <td>
                          <span class="stats-sublabel status-no-animals">No Animals</span>
                        </td>
                      </tr>
                    <?php endwhile; ?>
                  </tbody>
    ...
    <script>
        ...
        function updateStatsCards() {
            ...
            document.getElementById('totalClientsCount').textContent = totalVisible.toLocaleString();
            document.getElementById('compliedClientsCount').textContent = compliedCount.toLocaleString();
            document.getElementById('pendingClientsCount').textContent = pendingCount.toLocaleString();
            document.getElementById('noAnimalsClientsCount').textContent = noAnimalsCount.toLocaleString();
        }
        ...
        function applyFilters() {
            ...
            tableRows.forEach(row => {
                ...
                // Barangay filter
                if (showRow && barangayFilter !== '') {
                    const barangay = row.cells[2].textContent.trim();
                    if (barangay !== barangayFilter) {
                        showRow = false;
                    }
                }
            ...
            // Update stats cards after filtering
            updateStatsCards();
        }
