...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Removed last_vaccination field
    
    // Set weight based on animal type
    $weight = ($animal_type === 'Livestock') ? $_POST['weight'] : 0;
    
    // Prepare SQL statement - only update health_status and weight (for livestock)
    ...
    $stmt->bind_param("sdi", $health_status, $weight, $animal_id);
    
    // Execute the statement
    if ($stmt->execute()) {
        // Removed client type update - type column doesn't exist
