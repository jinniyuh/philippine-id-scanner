...
if (!empty($month)) {
    ...
    $monthCondition = "AND t.issued_date IS NOT NULL AND MONTH(t.issued_date) = '" . intval($month) . "' AND YEAR(t.issued_date) = YEAR(CURDATE())";
}
...
if ($status === 'all') {
    ...
    $query = "SELECT t.transaction_id, t.quantity, t.status, t.issued_date,
              ...
              JOIN clients c ON t.client_id = c.client_id
              JOIN pharmaceuticals p ON t.pharma_id = p.pharma_id
              WHERE t.status IN ('Approved', 'Issued') AND t.issued_date IS NOT NULL $monthCondition
              ORDER BY t.issued_date DESC";
    ...
    $query = "SELECT t.transaction_id, t.quantity, t.status, t.issued_date,
              ...
              JOIN clients c ON t.client_id = c.client_id
              JOIN pharmaceuticals p ON t.pharma_id = p.pharma_id
              WHERE t.status = 'Pending' AND t.issued_date IS NOT NULL $monthCondition
              ORDER BY t.issued_date DESC";
