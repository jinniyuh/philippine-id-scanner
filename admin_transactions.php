...
// --- UPDATED STATS CARDS ---
// Get current month filter from URL or default to all data
$selectedMonth = $_GET['month'] ?? '';
$monthCondition = '';
if (!empty($selectedMonth)) {
    // Filter by issued_date only since that's what we display
    $monthCondition = "AND issued_date IS NOT NULL AND MONTH(issued_date) = '" . intval($selectedMonth) . "' AND YEAR(issued_date) = YEAR(CURDATE())";
    // Debug: Log the month condition
    error_log("Month filter active: " . $selectedMonth . " | Condition: " . $monthCondition);
}

// Total Transactions
$transactionsCount = 0;
$resultTrans = $conn->query("SELECT COUNT(*) as cnt FROM transactions WHERE issued_date IS NOT NULL $monthCondition");
if ($resultTrans && $row = $resultTrans->fetch_assoc()) {
    $transactionsCount = $row['cnt'];
    // Debug: Log the transaction count
    error_log("Total transactions count: " . $transactionsCount . " | Month: " . $selectedMonth);
}

// Approved Transactions (Approved + Issued)
$approvedCount = 0;
$resultApproved = $conn->query("SELECT COUNT(*) as cnt FROM transactions WHERE status IN ('Approved', 'Issued') AND issued_date IS NOT NULL $monthCondition");
...
$pendingCount = 0;
$resultPending = $conn->query("SELECT COUNT(*) as cnt FROM transactions WHERE status = 'Pending' AND issued_date IS NOT NULL $monthCondition");
...
$pharmaCount = 0;
$resultPharma = $conn->query("SELECT SUM(quantity) as total_dispensed FROM transactions WHERE status IN ('Approved', 'Issued') AND issued_date IS NOT NULL $monthCondition");
...
// Month condition already defined above, use it here
...
$sql = "SELECT t.*, c.full_name AS client_name, p.name AS pharma_name, p.unit
        ...
        WHERE t.issued_date IS NOT NULL $monthCondition
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="dispensedMedicinesModalLabel">
                        <i class="fas fa-pills me-2"></i>All Dispensed Transactions
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
    ...
    $(document).ready(function() {
        ...
        function preloadTransactionData() {
            ...
            statuses.forEach(status => {
                fetch(`get_admin_transactions.php?status=${status}${monthParam}`)
                ...
                <tr>
                    <td colspan="4" class="text-center">
                        <div class="d-flex justify-content-center align-items-center py-4">
                            ...
                            <span>Loading dispensed medicines...</span>
                        </div>
                    </td>
                </tr>
            `;
            
            // Fetch individual dispensed transactions instead of aggregated data
            fetch(`get_admin_transactions.php?status=approved${monthParam}`)
                .then(response => response.json())
                .then(data => {
                    const tbody = document.getElementById('dispensedMedicinesBody');
                    tbody.innerHTML = '';
        ...
        document.getElementById('searchTransaction').addEventListener('input', function() {
            // Don't apply filters if month filter is active (server-side filtering)
        ...
        document.getElementById('filterMonth').addEventListener('change', function() {
            ...
            // Reload page with month filter to update stats cards
            if (selectedMonth) {
                window.location.href = `admin_transactions.php?month=${selectedMonth}`;
            } else {
        ...
        function applyFilters() {
            // Don't apply client-side filtering if month filter is active (server-side filtering)
            const urlParams = new URLSearchParams(window.location.search);
            const monthParam = urlParams.get('month');
            if (monthParam) {
                console.log('Month filter active, skipping client-side stats update');
                return;
            }
            ...
            tableRows.forEach(row => {
                ...
                // Month filter - improved logic
                let monthMatch = selectedMonth === '' || (() => {
                    if (selectedMonth === '' || !dateCell) return true;
        ...
        function updateStatsCards(totalTransactions, dispensedMedicines) {
            // Target the specific metric-value elements within the stats cards
            const totalCard = document.querySelector('[data-bs-target="#totalTransactionsModal"] .metric-value');
            const dispensedCard = document.querySelector('[data-bs-target="#dispensedMedicinesModal"] .metric-value');
        ...
        // Set selected month from URL parameter first
        const urlParams = new URLSearchParams(window.location.search);
        const monthParam = urlParams.get('month');
        if (monthParam) {
            document.getElementById('filterMonth').value = monthParam;
            // Don't override server-side stats when month filter is applied
            console.log('Month filter detected, using server-side stats');
        } else {
            // Only apply client-side filtering when no month filter is active
            applyFilters();
        }
    });
