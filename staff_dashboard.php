<?php
    ...
    // Pharmaceutical Requests Count (where status = 'Pending')
    $queryPharmaRequestsCount = "SELECT COUNT(*) AS pharmaRequestCount FROM pharmaceutical_requests WHERE status = 'Pending'";
    $resultPharmaRequestsCount = mysqli_query($conn, $queryPharmaRequestsCount);
    $rowPharmaRequestsCount = mysqli_fetch_assoc($resultPharmaRequestsCount);
    $pharmaRequestCount = $rowPharmaRequestsCount['pharmaRequestCount'];
    
    // Most Recent Pharmaceutical Request
    $queryRecentPharmaRequest = "SELECT pr.*, c.full_name FROM pharmaceutical_requests pr 
                                JOIN clients c ON pr.client_id = c.client_id 
                                ORDER BY pr.request_date DESC LIMIT 1";
    $resultRecentPharmaRequest = mysqli_query($conn, $queryRecentPharmaRequest);
    $recentPharmaRequest = $resultRecentPharmaRequest ? mysqli_fetch_assoc($resultRecentPharmaRequest) : null;
    ...
    <head>
        ...
        <style>
            ...
            /* Ensure proper dropdown separation */
...
<h2>Staff Dashboard</h2></div>
        <div class="staff-info">
            ...
            <div class="dropdown" id="notificationDropdown">
                <button class="notification-btn" id="notificationBtn" data-bs-toggle="dropdown" aria-expanded="false" data-bs-auto-close="outside">
                ...
                <ul class="dropdown-menu dropdown-menu-end notification-dropdown" style="width: 320px; max-height: 400px; overflow-y: auto; border-radius: 10px; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                <a href="staff_notifications.php" class="text-decoration-none">
                                    <div class="alert alert-primary mb-3 clickable-alert" style="border-radius: 10px; cursor: pointer; transition: all 0.3s ease; background-color: #e3f2fd !important; border-color: #bbdefb !important; color: #1565c0 !important;">
                                ...
                                <a href="staff_pharmaceutical_request.php" class="text-decoration-none">
                                    <div class="alert alert-primary mb-3 clickable-alert" style="border-radius: 10px; cursor: pointer; transition: all 0.3s ease; background-color: #e8eaf6 !important; border-color: #c5cae9 !important; color: #3f51b5 !important;">
                                        <div class="d-flex align-items-center mb-2">
                                            <i class="fas fa-syringe me-2"></i>
                                            <strong>Pharma Request</strong>
                                        </div>
                                        <p class="mb-0">
                                            <?php if ($pharmaRequestCount > 0): ?>
                                                <?php echo $pharmaRequestCount; ?> pending requests
                                                <?php if ($recentPharmaRequest): ?>
                                                    <br><small class="text-muted">Latest: <?php echo htmlspecialchars($recentPharmaRequest['full_name']); ?> - <?php echo htmlspecialchars($recentPharmaRequest['species']); ?></small>
                                                <?php endif; ?>
                                            <?php else: ?>
                                                No pending requests
                                            <?php endif; ?>
                                ...
                                <a href="staff_pharmaceuticals.php" class="text-decoration-none">
                                    <div class="alert alert-primary mb-0 clickable-alert" style="border-radius: 10px; cursor: pointer; transition: all 0.3s ease; background-color: #e0f2f1 !important; border-color: #b2dfdb !important; color: #00695c !important;">
                                        <div class="d-flex align-items-center mb-2">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (notificationBtn && notificationDropdown) {
                const notificationDropdownInstance = new bootstrap.Dropdown(notificationBtn);
                
                // Prevent event bubbling for notification dropdown
            ...
            if (avatarDropdownToggle && avatarDropdown) {
                ...
                const avatarDropdownInstance = new bootstrap.Dropdown(avatarDropdownToggle);
                
                // Add click event to button
                avatarDropdownToggle.addEventListener('click', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    avatarDropdownInstance.toggle();
                });
                ...
                // Prevent conflicts - only handle avatar dropdown clicks
                const avatarDropdownArea = document.querySelector('.dropdown:last-child');
                if (avatarDropdownArea) {
                    avatarDropdownArea.addEventListener('click', function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                        avatarDropdownInstance.show();
                    });
                }
            }
        });
