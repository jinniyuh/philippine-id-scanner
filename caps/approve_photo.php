...
try {
    ...
    $stmt = $conn->prepare("UPDATE animal_photos SET status = 'Approved', remarks = ?, approved_by = ?, approved_at = NOW() WHERE photo_id = ?");
    ...
    if ($stmt->execute()) {
        ...
        if ($photo_result->num_rows > 0) {
            ...
            $notification_message = "Your photo submission for " . $photo_data['species'] . " (" . $photo_data['animal_type'] . ") has been approved.";
            
            $notification_stmt = $conn->prepare("INSERT INTO notifications (user_id, message, type, status, created_at) VALUES (?, ?, 'photo_approval', 'Unread', NOW())");
        ...
        $photo_query->close();
        echo json_encode(['success' => true, 'message' => 'Photo approved successfully']);
    } else {
        echo json_encode(['success' => false, 'error' => 'Failed to approve photo']);
    }
