...
$res = $conn->query("SELECT AVG(quantity) as avg FROM transactions WHERE status='Approved' AND issued_date BETWEEN '$startDate' AND '$endDate'");
if($res) $summaryStats['avg_quantity'] = round($res->fetch_assoc()['avg'] ?? 0, 2);

// Monthly trend data for the selected period
$monthlyTrend = [];
$res = $conn->query("SELECT DATE_FORMAT(issued_date, '%Y-%m') as month, SUM(quantity) as total FROM transactions WHERE status='Approved' AND issued_date BETWEEN '$startDate' AND '$endDate' GROUP BY month ORDER BY month ASC");
if($res) while($row=$res->fetch_assoc()) $monthlyTrend[$row['month']] = (int)$row['total'];

// Client activity by barangay
$barangayStats = [];
$res = $conn->query("SELECT c.address as barangay, COUNT(DISTINCT t.client_id) as client_count, SUM(t.quantity) as total_quantity FROM transactions t JOIN clients c ON t.client_id = c.client_id WHERE t.issued_date BETWEEN '$startDate' AND '$endDate' GROUP BY c.address ORDER BY total_quantity DESC LIMIT 10");
if($res) while($row=$res->fetch_assoc()) $barangayStats[] = $row;

// Medicine category breakdown
$categoryStats = [];
$res = $conn->query("SELECT p.category, SUM(t.quantity) as total FROM transactions t JOIN pharmaceuticals p ON t.pharma_id = p.pharma_id WHERE t.status='Approved' AND t.issued_date BETWEEN '$startDate' AND '$endDate' GROUP BY p.category ORDER BY total DESC");
...
$res = $conn->query("SELECT DATE_FORMAT(issued_date, '%Y-%m') as month, SUM(quantity) as total FROM transactions WHERE status='Approved' AND issued_date >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH) GROUP BY month ORDER BY month ASC");
...
/* Summary Stats Cards */
.summary-stats { display: flex; gap: 24px; margin-bottom: 36px; flex-wrap: wrap; }
.metric-card { 
...
<div class="container-fluid">
    ...
    <div class="summary-stats">
        <div class="metric-card">
            <div class="metric-title"><i class="fas fa-file-invoice me-2"></i>Total Transactions</div>
        ...
        <div class="metric-card">
            <div class="metric-title"><i class="fas fa-pills me-2"></i>Medicines Dispensed</div>
        ...
        <div class="metric-card">
            <div class="metric-title"><i class="fas fa-users me-2"></i>Clients Served</div>
            <div class="metric-value"><?= number_format($summaryStats['total_clients'] ?? 0) ?></div>
            <div class="metric-detail">Unique beneficiaries</div>
        </div>
        <div class="metric-card">
            <div class="metric-title"><i class="fas fa-balance-scale me-2"></i>Avg. Quantity per Transaction</div>
...
<script src="https://kit.fontawesome.com/7e2e5e8e2e.js" crossorigin="anonymous"></script>
<script>
