...
$activity_message = "Printed Livestock Disseminated Report for period {$displayStartDate} to {$displayEndDate}";
logActivity($conn, $user_id, $activity_message);

// Get all disseminated livestock records with date filter
$sql = "SELECT c.full_name, 
               c.barangay as address,
               lp.species, lp.quantity
        FROM livestock_poultry lp
        LEFT JOIN clients c ON lp.client_id = c.client_id
        WHERE (lp.animal_type = 'Livestock' OR lp.animal_type = 'Both')
...
<head>
  <meta charset="utf-8">
  <title>Livestock Disseminated Report</title>
...
<?php if (!empty($data)): ?>
    <?php for ($page = 0; $page < $pages; $page++): ?>
        <div class="container-fluid p-4 print-page">
            ...
            <!-- Title + Print button (only on first page) -->
            <?php if ($page === 0): ?>
            <div class="d-flex justify-content-between align-items-center no-print">
              <div class="w-100 text-center">
                <h5 class="mb-0">LIVESTOCK DISSEMINATION REPORT</h5>
              </div>
            ...
            <?php else: ?>
            <div class="text-center" style="font-size:16px; font-weight:bold; margin-bottom:5px;">LIVESTOCK DISSEMINATION REPORT</div>
