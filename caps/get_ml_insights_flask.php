...
try {
    ...
    if (!isset($data['summary_metrics'])) {
        // Try to calculate accuracy from insights
        $overall_accuracy = 'N/A';
        if (isset($data['insights'])) {
            $accuracy_scores = [];
            
            // Check if we have accuracy data in insights
            foreach (['pharmaceutical_demand', 'livestock_population', 'poultry_population'] as $insight_type) {
                if (isset($data['insights'][$insight_type]['accuracy'])) {
                    $accuracy_scores[] = $data['insights'][$insight_type]['accuracy'];
                }
            }
