                                ...
                                <td>
                                    <button type="button" class="action-btn edit-btn" 
                                        onclick="editAnimal(
                                            ...
                                            '<?php echo date('Y-m-d', strtotime($livestock['last_vaccination'])); ?>'
                                        )">
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-hover">
                        ...
                        <?php if ($poultry_data && $poultry_data->num_rows > 0): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="6" class="text-center">No poultry records found</td></tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="modal-dialog modal-md">
        <form id="editAnimalForm" action="admin_update_animal.php" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3" id="edit_vaccination_group">
                    <label for="edit_last_vaccination" class="form-label">Last Vaccination</label>
                    <input type="date" class="form-control" id="edit_last_vaccination" name="last_vaccination">
                </div>
            </div>
...
<script>
  ...
  function updateSpeciesOptions() {
    ...
    // Toggle vaccination field visibility: show for Livestock, hide for Poultry
    const vaccinationGroup = document.getElementById('vaccination_group');
    const vaccinationInput = document.getElementById('last_vaccination');
    if (vaccinationGroup) {
      if (selectedType === 'Poultry') {
        vaccinationGroup.style.display = 'none';
        vaccinationInput.required = false;
        vaccinationInput.value = '';
      } else {
        vaccinationGroup.style.display = '';
        vaccinationInput.required = true;
      }
    }
  }
  ...
  function updateSpeciesOptionsEdit(callback) {
    ...
    // Toggle vaccination field visibility for edit: hide for Poultry
    const vaccinationGroup = document.getElementById('edit_vaccination_group');
    const vaccinationInput = document.getElementById('edit_last_vaccination');
    if (vaccinationGroup) {
      if (type === 'Poultry') {
        vaccinationGroup.style.display = 'none';
        vaccinationInput.required = false;
  ...
  function editAnimal(animal_id, type, species, weight, quantity, client_id, source, health_status, last_vaccination) {
    ...
    // Show/hide vaccination field based on animal type
    const vaccinationGroup = document.getElementById('edit_vaccination_group');
    const vaccinationInput = document.getElementById('edit_last_vaccination');
    ...
    if (type === 'Livestock') {
        ...
        vaccinationInput.required = true;
        vaccinationInput.value = last_vaccination;
    } else {
        vaccinationGroup.style.display = 'none';
        vaccinationInput.required = false;
        vaccinationInput.value = '';
  ...
  //     // For Poultry table: columns [Client, Species, Source, Barangay]
  //     filterTableRows(q, '#poultry .table', [0, 1, 3, 4]);
  //   });
  // }
