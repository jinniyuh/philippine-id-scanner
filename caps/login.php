  ...
  <script>
    ...
    async function scanValidId() {
      ...
      try {
        ...
        // Extract name from ID and auto-fill the form
        const nameFromId = extractFullNameFromOCR(ocrText) || '';
        const enteredName = document.getElementById('full_name').value || '';
        
        // Auto-fill the name field if empty
        if (!enteredName && nameFromId) {
          document.getElementById('full_name').value = nameFromId;
        }
        
        // Persist extracted name for server check (fallback to entered text if OCR couldn't parse)
        const hidden = document.getElementById('scanned_full_name');
        const finalNameForServer = nameFromId || enteredName || '';
        if (hidden) hidden.value = finalNameForServer;

        // Check if Bago resident (main requirement)
        if (isBagoResident) {
          idVerified = true;
          statusEl.className = 'form-text text-success';
          statusEl.textContent = '✅ ID verified! Bago resident confirmed. Please complete your registration details.';
          showAutoRegistrationMessage();
        } else {
          idVerified = false;
          statusEl.className = 'form-text text-danger';
          
          // Check if it's a specific other city
          const otherCities = [
          ...
          for (let city of otherCities) {
            if (ocrText.toUpperCase().includes(city.toUpperCase())) {
              detectedCity = city.charAt(0) + city.slice(1).toLowerCase();
              break;
            }
          }
          
          if (detectedCity) {
            statusEl.textContent = `❌ You are NOT a Bago City resident. You CANNOT register to our system. Your ID shows you are from ${detectedCity}, not Bago City.`;
          } else {
            statusEl.textContent = '❌ You are NOT a Bago City resident. You CANNOT register to our system. Only Bago City residents can register.';
          }
        }
      } catch (err) {
        console.error('Tesseract error', err);
        // On failure, do not verify
        idVerified = false;
        statusEl.className = 'form-text text-danger';
        statusEl.textContent = '❌ Failed to scan ID. Please retake a clearer photo of your ID.';
      }
    }
