...
<body>
    ...
    <script>
        ...
        function displayAnimalList(animals) {
            ...
            html += '<thead><tr><th style="width: 20%;">Species</th><th style="width: 15%;">Client</th><th style="width: 25%;">Health Status</th><th style="width: 12%;">Risk Level</th><th style="width: 13%;">Assessment</th><th style="width: 15%;">Actions</th></tr></thead><tbody>';
            ...
            animals.forEach(animal => {
                ...
                // Truncate health status for display
                let healthStatusDisplay = animal.health_status || 'Unknown';
                let fullHealthStatus = healthStatusDisplay;
                
                // If it's symptomatic, show a shorter version
                if (healthStatusDisplay.startsWith('Symptomatic:')) {
                    const symptoms = healthStatusDisplay.replace('Symptomatic:', '').trim();
                    const firstSymptom = symptoms.split(';')[0].trim();
                    healthStatusDisplay = `Symptomatic: ${firstSymptom.length > 30 ? firstSymptom.substring(0, 30) + '...' : firstSymptom}`;
                } else if (healthStatusDisplay.length > 40) {
                    healthStatusDisplay = healthStatusDisplay.substring(0, 40) + '...';
                }
                
                html += `
                    <tr>
                        <td><strong>${animal.animal_name || 'Unnamed'}</strong></td>
                        <td>${animal.client_name}</td>
                        <td>
                            <span class="badge bg-${animal.health_status === 'Healthy' ? 'success' : 'warning'}" 
                                  title="${fullHealthStatus}" 
                                  style="cursor: help; max-width: 100%; display: inline-block; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                ${healthStatusDisplay}
                            </span>
                        </td>
                        <td><span class="badge ${badgeClass}">${lastRiskLevel}</span></td>
                        <td><small>${lastAssessment}</small></td>
                        <td>
                            <button class="btn btn-sm btn-assess me-1" onclick="assessAnimal(${animal.animal_id})" title="Assess Animal">
                                <i class="fas fa-stethoscope"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-info" onclick="viewAssessment(${animal.animal_id})" title="View Assessment">
