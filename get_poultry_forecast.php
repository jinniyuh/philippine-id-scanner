...
try {
    ...
    if (count($historical) >= 6) {
        // Remove zeros for accuracy calculation (no data months)
        $nonZeroData = array_filter($historical, function($val) { return $val > 0; });
        
        if (count($nonZeroData) >= 6) {
            $forecaster = new VeterinaryForecaster($conn);
            $accuracy_result = $forecaster->validateForecastAccuracy(array_values($nonZeroData), 3);
            
            if ($accuracy_result && isset($accuracy_result['accuracy_percentage'])) {
                if ($accuracy_result['accuracy_percentage'] > 10) {
                    $accuracy = round($accuracy_result['accuracy_percentage'], 1) . '%';
                } else {
                    $accuracy = 'Low (<10%)';
                }
            }
    ...
    echo json_encode([
        'success' => false,
        'error' => 'Failed to generate poultry forecast',
