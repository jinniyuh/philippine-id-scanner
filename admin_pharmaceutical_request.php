...
if (isset($_POST['approve'])) {
    ...
    if (count($pharma_ids) === 0 || count($pharma_ids) !== count($quantities) || count($pharma_ids) !== count($units)) {
        ...
        header('Location: admin_pharmaceutical_request.php');
        exit();
    }
...
// Get current filters from URL or default to all data
$selectedMonth = $_GET['month'] ?? '';
$selectedSpecies = $_GET['species'] ?? '';

$monthCondition = '';
if (!empty($selectedMonth)) {
    $monthCondition = "AND MONTH(pr.request_date) = '" . intval($selectedMonth) . "' AND YEAR(pr.request_date) = YEAR(CURDATE())";
}

$speciesCondition = '';
if (!empty($selectedSpecies)) {
    $speciesCondition = "AND pr.species = '" . $conn->real_escape_string($selectedSpecies) . "'";
}

// Get statistics for the cards
$total_requests_result = $conn->query("SELECT COUNT(*) as count FROM pharmaceutical_requests pr WHERE pr.status = 'Pending' $monthCondition $speciesCondition");
$total_requests = $total_requests_result->fetch_assoc()['count'];

$total_clients_result = $conn->query("SELECT COUNT(DISTINCT c.client_id) as count FROM clients c JOIN pharmaceutical_requests pr ON c.client_id = pr.client_id WHERE pr.status = 'Pending' $monthCondition $speciesCondition");
...
$all_requests_list = $conn->query("SELECT pr.*, c.full_name, c.contact_number, c.barangay 
                                   ...
                                   WHERE pr.status = 'Pending' $monthCondition $speciesCondition
                                   ORDER BY pr.request_date DESC");

$total_clients_list = $conn->query("SELECT DISTINCT c.client_id, c.full_name, c.contact_number, c.barangay FROM clients c JOIN pharmaceutical_requests pr ON c.client_id = pr.client_id WHERE pr.status = 'Pending' $monthCondition $speciesCondition ORDER BY c.full_name ASC");
...
<head>
    ...
    <title>Admin - Pharma Requests</title>
...
<body>
                        ...
                        <?php while ($row = $requests->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                        ...
                                        <button class="btn btn-info btn-sm ms-1" data-bs-toggle="modal" data-bs-target="#symptomsModal" data-symptoms='<?= htmlspecialchars($row['symptoms'] ?? '', ENT_QUOTES); ?>' data-client='<?= htmlspecialchars($row['full_name'], ENT_QUOTES); ?>' data-species='<?= htmlspecialchars($row['species'] ?? '', ENT_QUOTES); ?>'>
    ...
    <script>
        ...
        document.getElementById('filterMonth').addEventListener('change', function() {
            ...
            // Build URL parameters preserving both month and species filters
            let urlParams = [];
            if (selectedMonth) {
                urlParams.push(`month=${selectedMonth}`);
            }
            if (selectedSpecies) {
                urlParams.push(`species=${encodeURIComponent(selectedSpecies)}`);
            }
            
            const url = urlParams.length > 0 
                ? `admin_pharmaceutical_request.php?${urlParams.join('&')}`
        ...
        function applyFilters() {
            ...
            tableRows.forEach(row => {
                ...
                if (searchMatch && speciesMatch && monthMatch) {
                    row.style.display = '';
                    visibleCount++;
                    visibleClients.add(row.cells[0].textContent.trim());
                } else {
            ...
            updateStatsCards(visibleCount, visibleClients.size);
        }
        
        function updateStatsCards(totalRequests, totalClients) {
            // Update the metric values
            const requestCard = document.querySelector('[data-bs-target="#allRequestsModal"] .metric-value');
            const clientCard = document.querySelector('[data-bs-target="#totalClientsModal"] .metric-value');
            
            if (requestCard) {
                requestCard.textContent = totalRequests.toLocaleString();
            }
            if (clientCard) {
                clientCard.textContent = totalClients.toLocaleString();
            }
        }
        ...
        // Update modals with filtered data
        document.getElementById('allRequestsModal').addEventListener('show.bs.modal', function() {
            updateAllRequestsModal();
        });
        
        document.getElementById('totalClientsModal').addEventListener('show.bs.modal', function() {
            updateTotalClientsModal();
        });
        
        function updateAllRequestsModal() {
            const tableRows = document.querySelectorAll('.requests-table tbody tr');
            const modalBody = document.querySelector('#allRequestsModal .modal-body tbody');
            ...
            tableRows.forEach(row => {
                if (row.style.display !== 'none' && row.cells.length > 1) {
                    ...
                    const species = row.cells[1].textContent;
                    const symptoms = row.cells[2].textContent;
                    ...
                    newRow.innerHTML = `
                        <td>${clientName}</td>
                        <td>${species}</td>
                        <td style="max-width: 200px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${symptoms}">${symptoms}</td>
                        <td>${requestDate}</td>
                    `;
            ...
            if (modalBody.children.length === 0) {
                modalBody.innerHTML = '<tr><td colspan="4" class="text-center">No requests found</td></tr>';
            }
        }
        
        function updateTotalClientsModal() {
            const tableRows = document.querySelectorAll('.requests-table tbody tr');
            const modalBody = document.querySelector('#totalClientsModal .modal-body tbody');
            ...
            tableRows.forEach(row => {
                if (row.style.display !== 'none' && row.cells.length > 1) {
                    ...
                    if (!uniqueClients.has(clientName)) {
                        ...
                        // Get client details from the row data or make an AJAX call
            ...
            if (modalBody.children.length === 0) {
                modalBody.innerHTML = '<tr><td colspan="3" class="text-center">No clients found</td></tr>';
            }
        }
        ...
        if (monthParam) {
            document.getElementById('filterMonth').value = monthParam;
        }
        if (speciesParam) {
            document.getElementById('filterSpecies').value = speciesParam;
        }
        
        // Initialize stats on page load
        document.addEventListener('DOMContentLoaded', function() {
            // Apply initial filters to set up stats
            applyFilters();
        });
    </script>
